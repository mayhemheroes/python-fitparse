#!/usr/bin/env bash
#
# mayhem/test.sh — RUN python-fitparse's own test suite (deps already installed by mayhem/build.sh)
# and emit a CTRF (ctrf.io) summary. exit 0 iff failed==0. PATCH-grade oracle: a no-op patch that
# neuters the library breaks the suite's behavioral assertions (asserted decoded values / golden
# CSVs), so it FAILS here (anti-reward-hacking).
#
# It does NOT compile — build.sh installed pytest + atheris into the in-image site dir and compiled
# the fitparse_run_tests ELF wrapper. We only RUN the suite. The suite is routed through that
...
# Put the in-image site dir (atheris + pytest) and the fitparse source tree on PYTHONPATH.
...
# (unittest TestCases, which pytest collects natively): test.py + test_records.py + test_utils.py.
LOG="$(mktemp)"
"$RUNNER" -p no:cacheprovider -o addopts= -o python_functions=test_* -q tests/test.py tests/test_records.py tests/test_utils.py 2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
