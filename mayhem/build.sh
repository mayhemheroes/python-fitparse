#!/usr/bin/env bash
#
# mayhem/build.sh — build the python-fitparse Atheris fuzz harness + its standalone reproducer,
...
#      resolves entirely from it (pip --no-index --find-links). fitparse itself has no runtime
#      deps and is exercised as its editable source tree (repo root on PYTHONPATH).
#   2. Compile launcher.c -> the ELF Mayhem target `fitparse_fuzzer` (Atheris is a Python
#      script; Mayhem needs an ELF cmd, and the gate needs DWARF < 4 — hence a compiled wrapper).
#   3. Build the same launcher as the standalone (run-once) reproducer `fitparse_fuzzer-standalone`.
#   4. Compile the pytest ELF runner wrapper `fitparse_run_tests` (so the sabotage oracle bites).
#
...
# wrapper, not the fuzzed Python; Atheris instruments the fitparse library itself at import time).
set -euo pipefail
...
#    manylinux wheel for this CPython. pytest runs fitparse's own unittest-based suite.
PKGS=(atheris pytest)
need_download=0
...
#    site dir holds atheris+pytest we SKIP the reinstall. fitparse itself stays the editable source
#    tree (repo root on PYTHONPATH) so a PATCH agent's edits under fitparse/ take effect with no
#    reinstall.
...
# fitparse is a top-level package at the repo root, so the repo root itself goes on PYTHONPATH.
PYRUN="$SITE:$SRC"
...
PYTHONPATH="$PYRUN" "$PY" -c 'import atheris, fitparse, pytest; print("imports OK: fitparse", fitparse.__version__)'
...
#    at run time (the Dockerfile sets ENV PYTHONPATH), so the Python side finds atheris + fitparse.
HARNESS="$SRC/mayhem/fuzz_reader.py"
echo ">> compiling fitparse_fuzzer (+ standalone) with DEBUG_FLAGS=$DEBUG_FLAGS"
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/fitparse_fuzzer"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/fitparse_fuzzer-standalone"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" "$SRC/mayhem/run_tests.c" -o "$SRC/fitparse_run_tests"

echo ">> build.sh complete"
ls -la "$SRC/fitparse_fuzzer" "$SRC/fitparse_fuzzer-standalone" "$SRC/fitparse_run_tests"
